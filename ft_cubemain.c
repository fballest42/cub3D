...
/*   Updated: 2020/12/04 09:05:45 by fballest         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	err->err6 = tex->rutano; //OJO ELIMINAR SOLO HABILITADO PARA PASAR TEX
...
int				ft_key_hook(int keycode, t_map *map)
{
	...
	mlx_hook(map->mlx_win, 3, 1L << 1, ft_keyrestore, map);
	if (keycode == A_KEY)
		printf("%s\n", "down");
